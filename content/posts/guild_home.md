...
本文主要记录了使用`ConcatAdapter`、`AsyncListDifferDelegationAdapter`、`MediatorLiveData`、`Flow`、静态代码检查工具`PMD`&`detekt`等对安卓pd 首页重构过程。
...
为此，我们完成了pd 首页内容架构的调整，可以下线`FacadeTopFrameController`、`ChannelListHeadView`相关代码，并使存量各个独立的置顶项`View`层代码改写为`RecyclerViewHolder`即可。
...
可以看到，旧版代码随着业务迭代，单个文件能用PMD工具扫描出上百的建议，代码质量令人堪忧。重构后，再用detekt工具结合内部的kotlin代码规范进行扫描，代码建议均控制在个位数（未清零原因：单个类函数数量较多）。
